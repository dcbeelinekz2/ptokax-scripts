...
--		[Command] = { function, Lowest Profile that can use this command (check Levels table), Description, Example, },
					...
					function(user,data)
						...
						if cat then
							if Category[string.lower(cat)] then
								Category[string.lower(cat)] = nil ShowEntry(1,table.getn(Entry),1,4,cat,"")
								SaveToFile(Settings.eFolder.."/"..Settings.cFile,Category,"Category")
								...
								if rel ~= "" then
									...
									if Exists == 1 then
										...
										if (string.len(rel) > Settings.EntrySize) then
											...
											table.insert( Entry, { user.sName, cat, rel, os.date(), } )
											SaveToFile(Settings.eFolder.."/"..Settings.rFile,Entry,"Entry")
											SendToAll(Settings.sBot,user.sName.." freshened up "..cat.." with: "..rel)
										end
									end
								else
		...
		[Settings.showCmd]	=	{
					function(user,data)
						...
						if cat then
							if Category[string.lower(cat)] then
								user:SendPM(Settings.sBot,ShowEntry(1, table.getn(Entry), 1, 3, cat, string.rep("\t",9).."Showing all "..cat.." ["..table.getn(Entry).."]"..string.rep("\t",7).."["..os.date().."]\r\n"))
		...
		[Settings.delCmd]	=	{
						...
						if rel then
							...
							elseif Category[string.lower(rel)] then
								if ShowEntry(1,table.getn(Entry),1,4,rel,"") == nil then
									ShowEntry(1,table.getn(Entry),1,4,rel,"")
									SaveToFile(Settings.eFolder.."/"..Settings.rFile,Entry,"Entry")
					...
					end, 4, "Deletes entry/ID/category", sPrefix..Settings.delCmd.." Matrix Revolutions; "..sPrefix..Settings.delCmd.." 5; "..sPrefix..Settings.delCmd.." Movies", },
		[Settings.delAllCmd]	=	{
					function(user,data)
						Entry = nil Entry = {} SaveToFile(Settings.rFile,Entry,"Entry")
	...
	if Settings.SendTo[sUser.iProfile] == 1 and Settings.SendRC == 1 then
		if sUser.bUserCommand then
			if sUser.bOperator then
				sUser:SendData("$UserCommand 1 3 Entry Point\\Add\\Category$<%[mynick]> !"..Settings.addCatCmd.." %[line:Category]&#124;")
				sUser:SendData("$UserCommand 1 3 Entry Point\\Delete\\Category$<%[mynick]> !"..Settings.delCatCmd.." %[line:Category]&#124;")
				sUser:SendData("$UserCommand 1 3 Entry Point\\Delete\\Specific ID$<%[mynick]> !"..Settings.delCmd.." %[line:ID]&#124;")
				sUser:SendData("$UserCommand 1 3 Entry Point\\Delete\\Specific Entry$<%[mynick]> !"..Settings.delCmd.." %[line:Entry Name&#124;")
				sUser:SendData("$UserCommand 1 3 Entry Point\\Delete\\Category Content$<%[mynick]> !"..Settings.delCmd.." %[line:Category Name&#124;")
				sUser:SendData("$UserCommand 1 3 Entry Point\\Delete\\All Entries$<%[mynick]> !"..Settings.delAllCmd.."&#124;")
			end
			sUser:SendData("$UserCommand 1 3 Entry Point\\Show\\Categories$<%[mynick]> !"..Settings.showCatCmd.."&#124;")
			sUser:SendData("$UserCommand 1 3 Entry Point\\Show\\All Entries$<%[mynick]> !"..Settings.showCmd.." all&#124;")
			sUser:SendData("$UserCommand 1 3 Entry Point\\Show\\Last "..Settings.iMax.." Entries$<%[mynick]> !"..Settings.showCmd.."&#124;")
			sUser:SendData("$UserCommand 1 3 Entry Point\\Show\\By Category$<%[mynick]> !"..Settings.showCmd.." %[line:Category]&#124;")
			sUser:SendData("$UserCommand 1 3 Entry Point\\Help$<%[mynick]> !"..Settings.helpCmd.."&#124;")
			sUser:SendData("$UserCommand 1 3 Entry Point\\Find$<%[mynick]> !"..Settings.findCmd.." %[line:String]&#124;")
			sUser:SendData("$UserCommand 1 3 Entry Point\\Add\\Entry$<%[mynick]> !"..Settings.addCmd.." %[line:Category] %[line:Entry]&#124;")
		end
	end
end
...
ShowEntry = function(Start,End,Order,Mode,String,Header)
	local msg, border = "\r\n",string.rep("-", 325)
	msg = msg.."     "..border.."\r\n"..Header.."     "..string.rep("-- --",65).."\r\n     Nr.\tDate - Time\t\tPoster\t\t\tCategory\t\t\tEntry\r\n"
	msg = msg.."     "..string.rep("-- --",65).."\r\n"
	for i = Start, End, Order do
		if Entry[i] then
			local tmp = msg.."     "..i..".\t"..Entry[i][4].."\t\t"..Entry[i][1]..DoTabs(1,CheckSize(Entry[i][1]))..Entry[i][2]..DoTabs(1,CheckSize(Entry[i][2]))..Entry[i][3].."\r\n"
			if Mode == 1 then
				msg = tmp
			elseif Mode == 2 then
				if Settings.Sensitive == 1 then 
					where = Entry[i][1]..Entry[i][2]..Entry[i][3]..Entry[i][4]
				else
					where = string.lower(Entry[i][1]..Entry[i][2]..Entry[i][3]..Entry[i][4]) String = string.lower(String)
				end
				if string.find(where,String) then msg = tmp end
			elseif Mode == 3 then
				if string.lower(Entry[i][2]) == string.lower(String) then msg = tmp end
			elseif Mode == 4 then
				if string.lower(Entry[i][2]) == string.lower(String) then table.remove(Entry,i) return nil end
			end
		end
	end
	msg = msg.."     "..border.."\r\n" return msg
