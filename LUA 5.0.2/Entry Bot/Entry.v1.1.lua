--/----------------------------------------------------------------------------
-- Entry Bot v1.0b by jiten - Following Release or Request bot (6/6/2005)
...
--		[Command] = { function, Lowest Profile that can use this command (check Levels table), Description, Example, },
		...
		[Settings.TopVotesCmd]	=	{
					function(user,data)
						...
						for j = 1, Settings.vMax do
							if Voting[j] then
								msg = msg.."     "..Voting[j][1]..".\t"..Voting[j][6].."\t"..Voting[j][5].."\t\t"..Voting[j][2]..DoTabs(1,CheckSize(Voting[j][2]))..Voting[j][3]..DoTabs(1,CheckSize(Voting[j][3]))..Voting[j][4]..DoTabs(1,CheckSize(Voting[j][4])).."\r\n"
							end
						end
						msg = msg.."     "..border.."\r\n"
						user:SendPM(Settings.sBot,msg)
					end, 1, "Top Entry Votes", sPrefix..Settings.TopVotesCmd, },
