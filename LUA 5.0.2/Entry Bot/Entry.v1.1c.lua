--/----------------------------------------------------------------------------
-- Entry Bot v1.1c by jiten - Following Release or Request bot (6/13/2005)
		...
		[Settings.addCatCmd]	=	{
					function(user,data)
						...
						if cat and date then
							...
							user:SendData(Settings.sBot,"*** Syntax Error: Type "..sPrefix..Settings.addCatCmd.." <category> <maximum time in days>")
						end
	...
	if Settings.SendTo[sUser.iProfile] == 1 and Settings.SendRC == 1 then
		if sUser.bUserCommand then
			if sUser.bOperator then
				sUser:SendData("$UserCommand 1 3 "..Settings.sMenu.."\\Add\\Category$<%[mynick]> !"..Settings.addCatCmd.." %[line:Category] %[line:maximum time in days]&#124;") 
