...
MaxShow = 50			--Number of entries to show
		...
		if IP then
			...
			if not (a == "" or b == "" or c == "" or d == "") then
				...
				if Exists == 1 then
					...
					user:SendData(sBot,"*** Error: There's no IP: "..IP.." in the Connection Logs.")
				end
			else
				user:SendData(sBot, "*** Syntax Error: Type "..LogCmd.." <IP>")
			end
		else
			local Structure = "\r\n<"..string.rep("-",45).."-[ Last ( "..table.getn(Log).." ) Connection Log Entries ] ----------->\r\n\r\n"
			for i = 1, MaxShow do Structure = Structure..Log[i].."\r\n" end
			Structure = Structure.."\r\n<"..string.rep("-",60).."-[ End of Connection Log ]--------------->"
			user:SendPM(sBot,Structure)
		end
		return 1
...
NewUserConnected = function(user,data)
	Logger(user,data,"Log In")
end

OpConnected = NewUserConnected

UserDisconnected = function(user,data)
	Logger(user,data,"Log Out")
end

OpDisconnected = UserDisconnected

Logger = function(user,data,log)
	...
	local userinfo = "- "..log.." > "..user.sName..", Profile: "..(GetProfileName(user.iProfile) or "Not Registered")..", IP: "..user.sIP..
	...
	", Slots: "..user.iSlots..", Hubs as normal User: "..(user.iNormalHubs or 0)..", Hubs as Registered User: "..(user.iRegHubs or 0)..
	", Hubs as Operator: "..(user.iOpHubs or 0)..", Total Hubs: "..(user.iHubs or 0)
