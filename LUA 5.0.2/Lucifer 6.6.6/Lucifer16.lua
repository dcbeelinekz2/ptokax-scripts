...
ToArrival = function(user, data)
	...
	if not user.bOperator then
		...
		if GetItemByName(to) and not GetItemByName(to).bOperator and PubCheck(user, data, msg, to, from) then return 1 end
	end
end
...
PubCheck = function(user, data, msg, to, from)
	...
	if adCheck(msg) ~= nil and Verify(userdata, msg) then
		...
		SendPmToOps(BotName, "User <"..user.sName.."> ("..user.sIP..") is "..DoDisc(user,msg).." for advertising "..tmp)
		return true
	end
	for key, value in controltrigs do
		if( string.find( string.lower(data), key) ) then SendPmToOps(BotName, "Control: User <"..user.sName.."> ("..user.sIP..") said "..tmp) end
	end
end
