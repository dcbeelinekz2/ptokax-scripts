...
-- Set action for advertising [ 0=Silent (only censors) / 1=Warn / 2=Disconnect / 3=Kick / 4=Ban / 5=TimeBan ]
Action = 0
...
PubCheck = function(user, data, msg, to, from)
	...
	if adCheck(msg) ~= nil and Verify(userdata, msg) then
		...
		SendTo("User <"..user.sName.."> ("..user.sIP..") is "..DoDisc(user,msg).." for advertising "..tmp)
		return true
	end
...
Verify = function(userdata, msg)
	...
	for value, key in Lucifer["adver"][1] do if (string.find(Lines, string.lower(value), 1, 1)) then return Lines end end
end
