--[[ 
 ...
 LUA 5 version by jiten 
	...
	Changed: User nicks don't count for Control messages - requested by GB (2/28/2006)
...
ChatArrival = function(user,data)
	...
	if tAllowed[user.iProfile] == 1 then
		...
		if PubCheck(user,msg) then return 1 end
	end
end
	...
	function(user,data)
		...
		if type and Lucifer[string.lower(type)] and trig and trig ~= "" then
			...
			if tLucifer[type][1][trig] then
				user:SendData(sBot,"*** Error: There is already a "..trig.." in the "..tLucifer[type][2].." list!")
			else
				Lucifer[type][1][trig] = 1 SaveToFile(fLucifer,Lucifer,"Lucifer")
...
ToArrival = function(user, data)
	...
	if tAllowed[user.iProfile] ~= 1 then
		...
		if GetItemByName(to) and tAllowed[GetItemByName(to).iProfile] ~= 1 and PubCheck(user, msg, to) then return 1 end
	end
end
