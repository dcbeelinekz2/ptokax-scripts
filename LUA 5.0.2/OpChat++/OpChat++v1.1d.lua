...
--## Settings ##-- 
sOpChat = "OpChat"					-- OpChat++ Bot Name
...
Main = function() 
	...
	if sendCustom == 1 then
		for usr, aux in pairs(arrInvite) do 
...
--	["invitelist"] = { function, 4 } -- All users with and above 4 (that would be Profile 1, 4, 0 and 5 in Levels table) can use this command.
	...
	function(user)
		...
		for usr, op in pairs(arrInvite) do 
...
SendTalkToOps = function(data) 
	...
	if (from ~= nil and talk ~= nil) then 
		SaveToLog(from, talk) 
		for aux, profile in pairs(GetProfiles()) do 
			for aux, usr in pairs(GetUsersByProfile(profile)) do 
...
SaveToLog = function(from, talk) 
	local usr,aux 
	for usr, aux in pairs(arrLog) do 
