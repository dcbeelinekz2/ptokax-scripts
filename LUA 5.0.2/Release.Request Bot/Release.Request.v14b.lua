-------/-------------------------------------------------------------------------------------------------------------------------
-- Release or Request bot v.1.4b by jiten (5/27/2005)
...
Main = function()
	...
	SetTimer(1000*60*60*12) StartTimer() sCheck(1)
	if Config.Mode == "Release" then if Settings.doPrefix ~= 1 then mPrefix = Settings.RelPrefix else mPrefix = "" end 
	elseif Config.Mode == "Request" then if Settings.doPrefix ~= 1 then mPrefix = Settings.ReqPrefix else mPrefix = "" end end
end
...
ChatArrival = function(sUser,sData) 
	...
	if sPrefix and Settings.tPrefixes[sPrefix] then
		...
		[mPrefix..Settings.addCmd] = {	
			function(user,data)
				...
				local linkCheck = function()
					...
					elseif (string.len(cat) > Settings.TypeSize) then
						...
						if ShowReleases(1,table.getn(Table),1,4,rel,"") ~= nil then
							...
							SendToAll(Settings.sBot, user.sName.." added a new "..Config.Mode..": "..rel..". For more details type: "..sPrefix..mPrefix..Settings.showCmd)
						else
				...
				if Config.Link == "on" then 
					if (link == nil) then user:SendData(Settings.sBot,"*** Error: Type "..sPrefix..mPrefix..Settings.addCmd.." <Type> <"..Config.Mode.."> <link> (Link is required)") else linkCheck() end
				else
					if (rel == nil or cat == nil) then user:SendData(Settings.sBot,"*** Error: Type "..sPrefix..mPrefix..Settings.addCmd.." <Type> <"..Config.Mode.."> <link> (Link is "..Optional.."optional)") else linkCheck() end
				end
			end, 1, "Adds a "..Config.Mode, sPrefix..mPrefix..Settings.addCmd.." Movie Blade3 http://www.blade.com (link is "..Optional.."optional)", },
		[mPrefix..Settings.delCmd] = {	
			function(user,data)
				...
				if i then
					...
					user:SendData(Settings.sBot,"*** Error: Type "..sPrefix..mPrefix..Settings.delCmd.." <"..Config.Mode.." number>")
				end
			end, 0, "Deletes single "..Config.Mode.."s", sPrefix..mPrefix..Settings.delCmd.." <"..Config.Mode..">", },
		[mPrefix..Settings.showCmd] = { 
			...
			end, 1, "Shows last "..Settings.iMax.." or all "..Config.Mode.."s", sPrefix..mPrefix..Settings.showCmd.."; "..sPrefix..mPrefix..Settings.showCmd.." all", },
		[mPrefix..Settings.findCmd] = { 
			...
			end, 1, "Find a "..Config.Mode.." by any string.", sPrefix..mPrefix..Settings.findCmd.." jiten", }, 
		[mPrefix..Settings.delAllCmd] = { 
			...
			end, 0, "Deletes all "..Config.Mode.."s", sPrefix..mPrefix..Settings.delAllCmd, },
			...
			function(user,data)
				...
				if not (set == nil or value == nil) then
					...
					elseif set == "release" then
						if value == "on" then
							Config.Mode = "Release" if Settings.doPrefix ~= 1 then mPrefix = Settings.RelPrefix else mPrefix = "" end sCheck(1)
							user:SendData(Settings.sBot,"Release Mode has been enabled.")
						elseif value == "off" then
							Config.Mode = "Request" if Settings.doPrefix ~= 1 then mPrefix = Settings.ReqPrefix else mPrefix = "" end sCheck(1)
	...
	if Settings.SendTo[sUser.iProfile] == 1 and Settings.SendRC == 1 then
		if sUser.bUserCommand then
			if sUser.bOperator then
				...
				sUser:SendData("$UserCommand 1 3 "..Config.Mode.." Bot\\Delete all$<%[mynick]> !"..mPrefix..Settings.delAllCmd.."&#124;|")
				sUser:SendData("$UserCommand 1 3 "..Config.Mode.." Bot\\Delete single$<%[mynick]> !"..mPrefix..Settings.delCmd.." %[line:Release Name]&#124;|")
				sUser:SendData("$UserCommand 0 3 |")
			end
			sUser:SendData("$UserCommand 1 3 "..Config.Mode.." Bot\\Show last "..Settings.iMax.."$<%[mynick]> !"..mPrefix..Settings.showCmd.."&#124;|")
			sUser:SendData("$UserCommand 1 3 "..Config.Mode.." Bot\\Show all<%[mynick]> !"..mPrefix..Settings.showCmd.." all&#124;|")
			...
			sUser:SendData("$UserCommand 1 3 "..Config.Mode.." Bot\\Help$<%[mynick]> !"..mPrefix..Settings.helpCmd.."&#124;|")
			sUser:SendData("$UserCommand 1 3 "..Config.Mode.." Bot\\Find$<%[mynick]> !"..mPrefix..Settings.findCmd.." %[line:String]&#124;|")
			sUser:SendData("$UserCommand 1 3 "..Config.Mode.." Bot\\Add$<%[mynick]> !"..mPrefix..Settings.addCmd.." %[line:Type] %[line:Release] %[line:Link]&#124;|")
		end
	end
end
...
sCheck = function(Mode)
	if Mode == nil then
		if Config.Mode == "Release" then SaveToFile(Settings.sFolder.."/"..Settings.fRelease,Release,"Release") elseif Config.Mode == "Request" then SaveToFile(Settings.rFolder.."/"..Settings.fRequest,Request,"Request") end
	elseif Mode == 1 then
		if Config.Mode == "Release" then SaveToFile(Settings.sFolder.."/"..Settings.fConfig,Config,"Config") elseif Config.Mode == "Request" then SaveToFile(Settings.rFolder.."/"..Settings.fConfig,Config,"Config") end
	end
end
