-------/-------------------------------------------------------------------------------------------------------------------------
-- Release or Request bot v.1.5 by jiten (5/27/2005)
...
ChatArrival = function(sUser,sData) 
	...
	if sPrefix and Settings.tPrefixes[sPrefix] then
		...
		local sHelpOutput = tmp.."\r\n\t[Mode / Cleaner / Link]: ["..Value.." / "..Config.Cleaner.." / "..Config.Link.."]\t\t\tRelease/Request v."..Settings.iVer.." by jiten\t\t\t["..Settings.Mode.." Mode]\r\n\t"..string.rep("-",220).."\r\n\tCommands:\t\tDescription:\t\tExample:".."\r\n\r\n"
		local tCmds = {
		...
		[mPrefix..Settings.delCmd] = {	
			function(user,data)
				...
				if i then
					...
					user:SendData(Settings.sBot,"*** Error: Type "..sPrefix..mPrefix..Settings.delCmd.." <"..Settings.Mode.." number>")
				end
		...
		[mPrefix..Settings.SetupCmd] = { 
			function(user,data)
				...
				if not (set == nil or value == nil) then
					...
					user:SendData(Settings.sBot,"*** Error: Type "..sPrefix..mPrefix..Settings.SetupCmd.." <release/link/cleaner> <on/off>")
				end
	...
	if Settings.SendTo[sUser.iProfile] == 1 and Settings.SendRC == 1 then
		if sUser.bUserCommand then
			if sUser.bOperator then
				...
				sUser:SendData("$UserCommand 1 3 "..Settings.Mode.." Bot\\Setup\\Release$<%[mynick]> !"..mPrefix..Settings.SetupCmd.." release %[line:on/off]&#124;|")
