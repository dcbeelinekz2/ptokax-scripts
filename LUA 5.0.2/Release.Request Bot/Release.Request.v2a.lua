-------/-------------------------------------------------------------------------------------------------------------------------
-- Release or Request bot v.2a by jiten (1/18/2006)
...
Settings = {
	...
	sSep = "\"",				-- Separator for Release/Request and Type (Default one is "). 
	...
	SendOnConnect = 0,			-- 1 = Send iMax Requests/Releases to every user on connect; 0 = Don't send
	...
	fReport = "rOffline.tbl",		-- File where the ReqDone Report Messages are stored
...
Release = {} Config = {} Request = {} ReqDone = {} tOffline = {}
...
Main = function()
	...
	if Settings.Mode == "Request" then
		...
		if loadfile(Settings.rFolder.."/"..Settings.fReport) then dofile(Settings.rFolder.."/"..Settings.fReport) end
	...
	Config.Cleaner = Config.Cleaner or "on" Config.Link = Config.Link or "off" sCheck(1)
...
sCheck = function(Mode)
	...
	if Mode == nil then
		...
		elseif Settings.Mode == "Request" then 
			...
			SaveToFile(Settings.rFolder.."/"..Settings.fReqDone,ReqDone,"ReqDone")
			SaveToFile(Settings.rFolder.."/"..Settings.fReport,tOffline,"tOffline") 
		end
	elseif Mode == 1 then
...
tCmds = {
	[mPrefix..Settings.addCmd] = {	
		function(user,data)
			local linkCheck = function(c,r,l)
				-- rel size checkpoint
				if (string.len(r) > Settings.RelSize) then
					user:SendData(Settings.sBot,"*** Error: The "..Settings.Mode.." Name can't have more than "..Settings.RelSize.." characters.")
				-- cat size checkpoint
				elseif (string.len(c) > Settings.TypeSize) then
					...
					if ShowReleases(1,table.getn(Table),1,4,r,"",Table) ~= nil then
						-- Release/Request Added
						if l == nil then l = "" end table.insert( Table, { user.sName, r, c, os.date(), l } ) sCheck() l = nil
						SendToAll(Settings.sBot, user.sName.." added a new "..Settings.Mode..": "..r..". For more details type: !"..mPrefix..Settings.showCmd)
					else
						user:SendData(Settings.sBot,"*** Error: There's already a "..Settings.Mode.." named: "..r..".")
					end
				end
			end
			local s,e,cat,rel,link = string.find(data,"%b<>%s+%S+%s+"..Settings.sSep.."(.*)"..Settings.sSep.."%s+"..Settings.sSep.."(.*)"..Settings.sSep.."%s*(.*)") 
			-- Data sent according to Link status
			if Config.Link == "on" then 
				if (link == nil) then user:SendData(Settings.sBot,"*** Error: Type !"..mPrefix..Settings.addCmd.." "..Settings.sSep.."Type"..Settings.sSep.." "..Settings.sSep..Settings.Mode..Settings.sSep.." <link> (Link is required)") else linkCheck(cat,rel,link) end
			else
				if (rel == nil or cat == nil) then user:SendData(Settings.sBot,"*** Error: Type !"..mPrefix..Settings.addCmd.." "..Settings.sSep.."Type"..Settings.sSep.." "..Settings.sSep..Settings.Mode..Settings.sSep.." <link> (Link is optional)") else linkCheck(cat,rel,link) end
			end
		end, 3, "Adds a "..Settings.Mode, "!"..mPrefix..Settings.addCmd.." "..Settings.sSep.."Movie"..Settings.sSep.." "..Settings.sSep.."Blade3"..Settings.sSep.." http://www.blade.com",
		"$UserCommand 1 3 "..Settings.Mode.." Bot\\Add$<%[mynick]> !"..mPrefix..Settings.addCmd.." "..Settings.sSep.."%[line:Type]"..Settings.sSep.." "..Settings.sSep.."%[line:Release]"..Settings.sSep.." %[line:Link]" },
	[mPrefix..Settings.delCmd] = {	
		function(user,data)
			local s,e,i = string.find(data,"%b<>%s+%S+%s+"..Settings.sSep.."(.*)"..Settings.sSep.."")
			if i then
				...
				if ShowReleases(1,table.getn(Table),1,3,i,"",Table) == nil then
					...
					user:SendData(Settings.sBot,Settings.Mode.." "..Settings.sSep..i..Settings.sSep.." was deleted succesfully!")
				else
					user:SendData(Settings.sBot,"*** Error: There is no "..Settings.Mode..": "..i..".")
				end
			else
				user:SendData(Settings.sBot,"*** Error: Type !"..mPrefix..Settings.delCmd.." "..Settings.sSep..Settings.Mode..Settings.sSep)
			end
		end, 4, "Deletes single "..Settings.Mode.."s", "!"..mPrefix..Settings.delCmd.." "..Settings.sSep..Settings.Mode..Settings.sSep,
		"$UserCommand 1 3 "..Settings.Mode.." Bot\\Delete single$<%[mynick]> !"..mPrefix..Settings.delCmd.." "..Settings.sSep.."%[line:Release Name]"..Settings.sSep },
	...
	[mPrefix..Settings.findCmd] = { 
		function(user,data)
			local s,e,str = string.find(data,"%b<>%s+%S+%s+"..Settings.sSep.."(.*)"..Settings.sSep)
			if str then
				-- Find
				...
				user:SendData(Settings.sBot,"*** Error: Type !"..mPrefix..Settings.findCmd.." "..Settings.sSep.."string"..Settings.sSep)
			end
		end, 1, "Find a "..Settings.Mode.." by any string.", "!"..mPrefix..Settings.findCmd.." "..Settings.sSep.."jiten"..Settings.sSep,
		"$UserCommand 1 3 "..Settings.Mode.." Bot\\Find$<%[mynick]> !"..mPrefix..Settings.findCmd.." "..Settings.sSep.."%[line:String]"..Settings.sSep }, 
	...
	[mPrefix..Settings.helpCmd] = { 
		function(user)
			local sHelpOutput = "\r\n\t"..string.rep("-", 220).."\r\n\t[Cleaner / Link]: ["..Config.Cleaner.." / "..Config.Link.." : "..Settings.iClean.." days]\t\tRelease/Request v."..Settings.iVer.." by jiten\t\t\t["..Settings.Mode.." Mode]\r\n\t"..string.rep("-",220).."\r\n\tCommands:\t\tDescription:\t\tExample:".."\r\n\r\n"
		...
		function(user,data)
			...
			if Settings.Mode == "Request" then
				if i then
					...
					user:SendData(Settings.sBot,"*** Error: Type !"..mPrefix..Settings.rDoneCmd.." "..Settings.sSep.."Request"..Settings.sSep)
				end
			else
				user:SendData(Settings.sBot,"*** Error: This command is only available in Request Mode.")
			end
		end, 1, "Fills up a Request", "!"..mPrefix..Settings.rDoneCmd.." "..Settings.sSep.."Request"..Settings.sSep,
		"$UserCommand 1 3 "..Settings.Mode.." Bot\\Fill up request$<%[mynick]> !"..mPrefix..Settings.rDoneCmd.." "..Settings.sSep.."%[line:Request Name]"..Settings.sSep},
...
NewUserConnected = function(sUser)
	...
	if Settings.SendOnConnect == 1 then
		sUser:SendPM(Settings.sBot,ShowReleases(table.getn(Table) - Settings.iMax + 1, table.getn(Table), 1, 1, false, string.rep("\t",9).."Last "..Settings.iMax.." "..Settings.Mode.."s"..string.rep("\t",8).."["..os.date().."]\r\n",Table))
	end
	-- Sending RightClick
	...
	-- Offline ReqDone Report
	if tOffline[sUser.sName] then sUser:SendPM(Settings.sBot,tOffline[sUser.sName]) tOffline[sUser.sName] = nil sCheck() end
end
	...
	for i = a, z, x do
		if tbl[i] then
			...
			elseif mode == 3 then
				if tbl[i][2] == str then 
					if user then 
						table.insert( ReqDone, { user, tbl[i][2], tbl[i][3], os.date(), tbl[i][5] } ) 
						-- Send/Store ReqDone Report
						local msg = "Your request \""..tbl[i][2].."\" has been filled by "..user.."."
						if GetItemByName(tbl[i][1]) then
							GetItemByName(tbl[i][1]):SendPM(Settings.sBot,msg)
						else
							tOffline[tbl[i][1]] = msg
						end
					end 
					table.remove(tbl,i)
					return nil
