...
function TrivEx:ErrorReport(curUser,data)
	if self:AllowedProf("Normal",curUser) then
		...
		if nr and report then
			table.insert(Report,{nr,report}) TrivEx:WriteTable(Report,"Report",self._Sets.folder.."/ErrorReport.tbl")
	...
	if curUser then
		...
		if (self.nick == nick) then
			...
			if (self.streak > TrivEx._Scores[curUser.sName].Streak)  then
				TrivEx:SendToPlayers(nick..", you broke your current streak of "..TrivEx._Scores[curUser.sName].Streak..".")
