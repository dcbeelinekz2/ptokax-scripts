	...
	if user.bUserCommand then
		for a,b in chatrooms.items do
			if b.groups[user.iProfile] or b.members[user.sName] then
				for i,v in tCmds do if(v.tLevels[user.iProfile]) then user:SendData(v.tRC) end end break
			end
		end
	end
end
...
--[[	Commands Structure:
	...
	[Commands.Status] = { 
		tFunc = function(user, data, tmp, to)
			...
			for v,i in tmp.groups do groups = groups.."\r\n\t• "..(GetProfileName(v) or "Unreg") end
			for v,i in tmp.hide do hide = hide.."\r\n\t• "..(GetProfileName(v) or "Unreg") end
