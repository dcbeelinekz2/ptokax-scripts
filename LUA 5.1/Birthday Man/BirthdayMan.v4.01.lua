--[[ 
	...
	Birthday Man v.4.01 - LUA 5.0/5.1 by jiten (13-04-2006)
		...
		tFunc = function(user,data)
			...
			if nick then
				if tBirthday[string.lower(nick)] then
					...
					user:SendMessage(Settings.sBot,"*** Error: "..nick.."'s Birthday hasn't been added to the DB yet!")
				end
			else
				user:SendMessage(Setings.sBot,"*** Syntax Error: Type !delbirthday <nick>")
			end
		end,
		tLevels = {
			[0] = 1, [5] = 1,
...
AddBirth = function(user,nick,args,d,m,y)
	if tonumber(y) > Settings.iMin then
		...
		"know "..nick.." was a "..Zodiac(tonumber(os.date("%m",tBirthday[nick].iJulian))).."!")
