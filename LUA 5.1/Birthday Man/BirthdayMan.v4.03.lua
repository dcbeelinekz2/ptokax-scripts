--[[ 
	...
	Birthday Man v.4.03 - LUA 5.0/5.1 by jiten (4/15/2006)
	...
	Fixed: !bornon Year function - thanks to Walazo (4/15/2006).
		...
		tFunc = function(user)
			if next(tBirthday) then
				...
				for i,v in pairs(tBirthday) do
					...
					sMsg = sMsg.."\t"..n.."\t"..sStatus.."\t\t"..sZodiac.."\t\t"..sWDay.."\t\t"..sDate.."\t\t"..
					string.upper(string.sub(i,1,1))..string.sub(i,2,string.len(i)).."\r\n"
				end
					...
					if i ~= string.lower(user.sName) then
						for a,b in pairs(tTable) do
							...
							local tFunction = function()
								local nick = string.upper(string.sub(i,1,1))..string.sub(i,2,string.len(i))
								if tSame[b.sDesc] then tSame[b.sDesc] = tSame[b.sDesc].."; "..nick else tSame[b.sDesc] = nick end
							end
							if b.bExtend then
...
AddBirth = function(user,nick,args,d,m,y)
	if tonumber(y) > Settings.iMin then
		...
		tBirthday[string.lower(nick)] = { iJulian = os.time(tTable), iAdjust = tmp }
		...
		"know "..nick.." was a "..Zodiac(tBirthday[string.lower(nick)].iJulian).."!")
			...
			local tTable = {
				["OnConnect"] = function()
					if i ~= string.lower(user.sName) then
						...
						user:SendData(Settings.sBot,"*** It's "..string.upper(string.sub(i,1,1))..
						string.sub(i,2,string.len(i)).."'s Birthday today! :D He/She is turning "..
