...
tSettings = {
	...
	bLocked = "off",		-- Default Locked Mode (on/off) - If on, only members/groups selected by owner are allowed.
	bResetAwayOnConnect = "off",	-- Reset away status on connect (on/off)
	tAllowed = {
		...
		if tmp.members[user.sName] then
			...
			local Messager = function()
				if tmp.autologin == "on" then
					...
					" to leave, and !"..Commands.Members.." to see who's invited!")
...
NewUserConnected = function(user)
	...
	if user.bUserCommand then
		...
		elseif tCmds[Commands.MakeChat].tLevels[user.iProfile] then
			user:SendData(tCmds[Commands.MakeChat].tRC)
		end
	end
end
