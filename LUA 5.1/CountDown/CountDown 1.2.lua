--[[
	...
	CountDown Bot 1.2 - LUA 5.0/5.1 version by jiten and TiMeTrAVelleR
...
Main = function()
	...
	SetTimer(100 * 1000); StartTimer(); Sync()
	local tmp = TimeLeft()
...
Sync = function()
	...
	if iDiff > 0 then
		...
		if T.day-1 ~= 0 then
			...
			local tTable = {
				[3600] = { fAdjust = T.min*60 + T.sec, iTimer = 3600 },
				[900] = { fAdjust = math.mod(T.min, 15)*60 + T.sec, iTimer = 900 },
				[300] = { fAdjust = math.mod(T.min, 5)*60 + T.sec, iTimer = 300 },
				[60] = { fAdjust = math.mod(T.min, 1)*60 + T.sec, iTimer = 60 },
				[15] = { fAdjust = math.mod(T.sec, 15), iTimer = 15 },
				[10] = { fAdjust = math.mod(T.sec, 5), iTimer = 5 },
				[1] = { fAdjust = math.mod(T.sec, 1), iTimer = 1 },
			}
			for i,v in pairs(tTable) do
				if iDiff > i then
					iAdjust = v.fAdjust; if iAdjust ~= 0 then SetTimer(iAdjust*1000) else SetTimer(v.iTimer*1000) end
				end
			end
		end
	end
