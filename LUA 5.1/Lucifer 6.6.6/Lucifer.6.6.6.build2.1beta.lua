	...
	fFunction = function(user,data)
		...
		if type and tLucifer[string.lower(type)] and trig and trig ~= "" then
			...
			if tLucifer[type][1][trig] then
				...
				tLucifer[type][1][trig] = 1; tFunctions.SaveToFile(tSettings.sTrigsFile,tLucifer,"tLucifer")
...
-- Command parsing
tFunctions.ParseCommands = function( ... )
	for i = 1, select("#", ... ) do
		local user, data = select(i, ... )
...
-- Pub checker
tFunctions.PubCheck = function( ... )
	for i = 1, select("#", ... ) do
		local user, msg, to = select(i, ... )
...
tFunctions.bUnSafe = function(Lines)
	local Lines = tostring(Lines)
	for value,i in pairs(tLucifer["safe"][1]) do
		for site, index in pairs(tLucifer["site"][1]) do
			if string.match(Lines,value,1) or string.match(Lines,site,1) then
				return false
			end
		end
	end
...
tFunctions.Verify = function( ... )
	for i = 1, select("#", ... ) do
		...
		for value, key in pairs(tLucifer["adver"][1]) do if (string.match(Lines, string.lower(value), 1)) then return value end end
	end
end

-- Advertise handling
tFunctions.DoDisc = function( ... )
	for i = 1, select("#", ... ) do
		local user, msg = select(i, ... )
		local tTable = {
