...
ChatArrival = function(user, data)
	...
	if (to and to ~= tSettings.sBot) or not to then
		...
		if tSettings.tBlockedProfiles[user.iProfile] and PubCheck(user, msg, GetItemByName(to)) then
			return 1
		end
	end
...
PubCheck = function(user, msg, to)
	...
	if to then userdata = to.sName.." "..user.sName; tmp = "to <"..to.sName.."> this: "..msg else userdata = user.sName end
