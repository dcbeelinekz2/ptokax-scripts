--[[
	...
	Release/Request Bot v3 - LUA 5.0.2/5.1 by jiten (3/26/2006)
...
tConfig = {}; tFilled = {}; tOffline = {}; tVote = {}
...
Main = function()
	...
	SetTimer(Settings.iCleanDelay*60*60*1000); if Settings.bTimer then StartTimer() end;
	if tSetup[string.lower(Settings.sMode)] then
		local tmp = tSetup[string.lower(Settings.sMode)]
		tTable = {}; tFunctions:load()
	end
...
tCmds = {
	...
	[Settings.voteCmd] = {
		...
		tRC = "Vote\\"..Settings.sMode.."$<%[mynick]> !"..tPrefix..Settings.voteCmd.." "..Settings.sSep.."%[line:Category]"..Settings.sSep.." %[line:ID]"
	},
	...
	[Settings.delCatCmd] = {
		...
		tRC = "Delete\\Category$<%[mynick]> !"..tPrefix..Settings.delCatCmd.." "..Settings.sSep.."%[line:Category]"..Settings.sSep..""
	},
	[Settings.showCatCmd] =	{
		tFunc = function(user)
	...
	[Settings.addCmd] = {	
		...
		tRC = "Add\\"..Settings.sMode.."$<%[mynick]> !"..tPrefix..Settings.addCmd.." "..Settings.sSep.."%[line:Type]"..Settings.sSep.." "..Settings.sSep.."%[line:Release]"..Settings.sSep.." %[line:Link]"
	},
	[Settings.delCmd] = {	
		tFunc = function(user,data)
			...
			if cat and rel then
				...
				user:SendData(Settings.sBot,"*** Syntax Error: Type !"..tPrefix..Settings.delCmd.." "..Settings.sSep.."category"..Settings.sSep.." <ID> / category>")
			end
		end, 
		tLevels = {
			[0] = 1,
		...
		sExample = "\t!"..tPrefix..Settings.delCmd.." "..Settings.sSep..Settings.sMode..Settings.sSep,
		tRC = "Delete\\"..Settings.sMode.."$<%[mynick]> !"..tPrefix..Settings.delCmd.." "..Settings.sSep.."%[line:Release Name]"..Settings.sSep
	},
	[Settings.showCmd] = { 
		tFunc = function(user,data)
	...
	[Settings.rDoneCmd] = { 
		...
		sExample = "\t!"..tPrefix..Settings.rDoneCmd.." "..Settings.sSep.."Request"..Settings.sSep,
		tRC = "Fill\\Up Request$<%[mynick]> !"..tPrefix..Settings.rDoneCmd.." "..Settings.sSep.."%[line:Request Name]"..Settings.sSep,
		bSpecial = true
...
tFunctions.GetRC = function(user,tTable)
	for i,v in pairs(tCmds) do
		if tCmds[i].tLevels[user.iProfile] then 
			if (string.lower(Settings.sMode) == "release" and i ~= Settings.rDoneCmd) or string.lower(Settings.sMode) == "request" then
				table.insert(tTable,v.tRC)
			end
		end
	end
end
