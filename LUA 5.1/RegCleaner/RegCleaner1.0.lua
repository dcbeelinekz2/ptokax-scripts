--[[
	...
	RegCleaner 1.0 - LUA 5.0/5.1 by jiten (9/13/2006)
	¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯
...
NewUserConnected = function(user)
	...
	if tUsers[user.sName] then
		-- Delete and save
		tUsers[user.sName] = nil; SaveToFile(tUsers, tSettings.fUser)
	end
end
...
UserDisconnected = function(user)
	...
	if tUsers[user.sName] then
		-- Replace and save
		tUsers[user.sName] = os.time(os.date("!*t")); SaveToFile(tUsers, tSettings.fUser)
	end
end
			...
			if nick then
				...
				if string.lower(nick) == string.lower(user.sName) then
					...
					if GetItemByName(nick) then
						...
						-- If in DB
						if tUsers[nick] then
							user:SendMessage(tSettings.sBot, "*** "..nick.." was last seen on: "..os.date("%x %X", tUsers[nick]))
						else
		...
		fFunction = function()
			...
			for prof, v in pairs(tSettings.tProfiles) do
				...
				for i, nick in ipairs(GetUsersByProfile(GetProfileName(prof))) do
					...
					if tUsers[nick] then
						-- If not immune
						if not tImmune[nick] then
							-- Hasn't logged in for more than specified weeks
							if os.time(os.date("!*t")) > tUsers[nick] + tSettings.tProfiles[prof]*7*24*60*60 then
								-- Remove from table and delete account
								tUsers[nick] = nil; DelRegUser(nick); 
								iCleaned = iCleaned + 1
							end
						end
					else
						-- Add to table
						tUsers[nick] = os.time(os.date("!*t"))
					end
					-- Collect garbage
