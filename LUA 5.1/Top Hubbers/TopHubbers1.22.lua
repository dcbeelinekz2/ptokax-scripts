--[[
	...
	TopHubbers 1.22 - LUA 5.0/5.1 by jiten
...
OnExit = function()
	SaveToFile(tSettings.fOnline, tOnline, "tOnline")
end
	...
	tophubbers = {
		fFunction = function(user, data)
			if next(tOnline) then
				...
				user:SendData(tSettings.sBot, "*** Error: Top Hubbers' table is currently empty!")
			end
		end,
		tLevels = {
		...
		fFunction = function(user, data)
			...
			if nick then 
				if tOnline[nick] then
					...
					if iAverage < 1 then iAverage = 1 end
					user:SendData(tSettings.sBot, "*** "..nick.."'s Total uptime: "..
					MinutesToTime(tOnline[nick].TotalTime, true).."; Daily average uptime: "..
					MinutesToTime((tOnline[nick].TotalTime/iAverage), true))
				else
					user:SendData(tSettings.sBot, "*** Error: No record found for '"..nick.."'!")
				end
			else
				user:SendData(tSettings.sBot, "*** Syntax Error: Type !hubtime <nick>")
			end
		end,
		tLevels = { 
			[0] = 1, [1] = 1, [4] = 1, [5] = 1,
