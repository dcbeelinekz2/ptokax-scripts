--[[
	...
	TopHubbers 1.1b - LUA 5.1 by jiten (3/5/2006)
		...
		tFunc = function(user,data)
			if next(tOnline) then
				...
				for v,i in pairs(tCopy) do
					...
					msg = msg.."\t"..v..".    "..tFunctions.SecondsToTime(i.iTotalTime).."\t"..
					math.floor(math.mod(i.iSessionTime/1, 60)).." min\t"..i.sEnter.."\t"..
					i.sLeave.."\t"..sStatus.."\t"..i.sNick.."\r\n"
				end
...
tFunctions = {
	...
	SecondsToTime = function(iSeconds)
		...
		return string.format("%i month(s), %i day(s), %i hour(s), %i minute(s)", iSeconds/518400, iSeconds/86400, tTime.hour, tTime.min)
	end,
