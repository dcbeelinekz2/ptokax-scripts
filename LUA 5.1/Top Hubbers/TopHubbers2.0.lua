--[[
	...
	TopHubbers 2.0 - LUA 5.0/5.1 by jiten
	¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯
...
tSettings = {
	...
	tProfiles = { [0] = 1, [1] = 1, [2] = 1, [3] = 1, [4] = 1, [5] = 1 },
...
Main = function()
	...
	string.gfind = (string.gfind or string.gmatch); SetTimer(60*1000); StartTimer()
end 
	...
	if tSettings.tProfiles[user.iProfile] and tSettings.tProfiles[user.iProfile] == 1 then
		...
		if tOnline[user.sName] then
			...
			if tSettings.bWarning then
				...
				local iAverage = os.difftime(os.time(os.date("!*t")), tOnline[user.sName].Julian)/(60*60)
...
UserDisconnected = function(user)
	...
	if tSettings.tProfiles[user.iProfile] and tSettings.tProfiles[user.iProfile] == 1 and tOnline[user.sName] then
		-- Log date
...
tCommands = {
	tophubbers = {
		...
		tRC = { { "Show Top 20 hubbers", "" }, { "Show Top x-y Hubbers", " %[line:x-y]" } }
	},
	hubtime = {
		fFunction = function(user, data)
			-- Parse nick
...
GetRank = function(nick)
	...
	if tSettings.bAverage then
		-- Average uptime in days
		iAverage = os.difftime(os.time(os.date("!*t")), tOnline[nick].Julian)/(60*60*24)
		if iAverage < 1 then iAverage = 1 end
	end
	-- For each rank
	for n in ipairs(tSettings.tRanks[string.lower(tSettings.sCriteria)]) do
		...
		for i, v in string.gfind(tSettings.tRanks[string.lower(tSettings.sCriteria)][n][2], "(%d+)(%w)") do
			-- Process
			if i and tTime[v] then iTime = iTime + i*tTime[v] end
		end
		local iValue = tOnline[nick].TotalTime
		-- Average
		if tSettings.bAverage then iValue = iValue/iAverage end
		-- Process rank if user hasn't logged in for the first time today
		if os.date("%d%m%y", tOnline[nick].Julian) ~= os.date("%d%m%y") and iValue > iTime then
