--[[
	...
	Vote Bot 1.02 - LUA 5.0/5.1 by jiten (7/11/2006)
	¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯
	...
	Added: Option to show all poll with its content - requested by TT; 
	Added: Poll options are shown in the Options' Box - requested by (uk)jay;
	Changed: Summary of availables replaced with all votes per poll on Connect (7/11/2006)
					...
					if v.bExtend then
						...
						for iIndex, tValue in pairs(tPoll) do
							...
							for sOption in pairs(tValue.tVotes) do
								-- Build
								if sMsg then sMsg = sMsg.."; "..sOption else sMsg = sOption end
								local sRC = string.gsub(b[2], "{}", tValue.sPoll.."\\"..sOption)
							end
...
tCommands = {
	...
	[tVote.sAddOption] = {
		...
		tRC = { { "Add Option to a Poll", " %[line:Poll ID] %[line:Option]" } },
	},
}
