...
setArrival("OnStartup", function()
	if loadfile("scripts/"..Settings.fBirth) then dofile("scripts/"..Settings.fBirth) end
	if Settings.bReg then RegBot(Settings.sBot,"","") end
...
setArrival("OpConnected", function(user)
	OnConnect(user)
end)
			...
			if args then
				...
				if d and m and y and nick then
					if tBirthday[nick] then
						...
						user:SendData(Settings.sBot,"*** Error: I already have "..nick.."'s Birthday on "..sDate..
						"! If it wasn't correctly set, ask Operators for help!")
					else
						AddBirth(user,nick,args,d,m,y)
					end
				else
					user:SendData(Settings.sBot,"*** Error: Birthday syntax must be: <nick> dd/mm/yyyy")
				end
			else
				user:SendData(Settings.sBot,"*** Syntax Error: Type !addbirthday <nick> dd/mm/yyyy")
			end
		end,
		tLevels = {
			[4] = 1, [5] = 1, [6] = 1
		...
		tFunc = function(user,data)
			...
			if nick then
				if tBirthday[string.lower(nick)] then
					tBirthday[string.lower(nick)] = nil; vDC.SaveFile(tBirthday,"tBirthday","scripts/"..Settings.fBirth)
					user:SendData(Settings.sBot,"*** Success: "..nick.."'s Birthday was successfully deleted from the DB!")
				else
					user:SendData(Settings.sBot,"*** Error: "..nick.."'s Birthday hasn't been added to the DB yet; or there's a Syntax Error: Type !delbirthday <nick>")
				end
			else
				user:SendData(Settings.sBot,"*** Syntax Error: Type !delbirthday <nick>")
			end
		end,
		tLevels = {
			[4] = 1, [5] = 1, [6] = 1
		...
		tFunc = function(user)
			if tBirthday[user.sName] then
				if SameBDay(user,tBirthday[user.sName].iJulian) ~= "" then
					...
					user:SendData(Settings.sBot,"*** Error: There are no common Birthday fields!") 
				end
			else 
				user:SendData(Settings.sBot,"*** Error: Please add your Birthday before using this command. Type !"..
				"bhelp for more details!")
...
AddBirth = function(user,nick,args,d,m,y)
	if tonumber(y) > Settings.iMin then
		...
		vDC.SaveFile(tBirthday,"tBirthday","scripts/"..Settings.fBirth)
		local sDate, sWDay, sZodiac = JulianToDate(tBirthday[nick])
		user:SendData(Settings.sBot,"*** "..nick.."'s Birthday is on "..sDate.."! I didn't "..
